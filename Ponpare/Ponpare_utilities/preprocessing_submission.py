...
def skim_visit_file():
    """ Some coupons are present in the _visit_ file but not in the _list_ file.
    ...
    coupon_list_train = pd.read_csv("../Data/Data_translated/coupon_list_train_translated.csv")
    ...
    #Find the pathological coupons
    list_row_visit = train_visit_df["VIEW_COUPON_ID_hash"].values 
    list_view_no_list = np.array(list_view_no_list)
    list_good_indices = np.in1d(list_row_visit, list_view_no_list, invert = True)

    #Save the skimmed data
    train_visit_df = train_visit_df.iloc[list_good_indices, :]
    train_visit_df.to_csv("../Data/Data_translated/coupon_visit_train_translated.csv", index = False)


def create_dict_user_list():
    ...
    d_user_list = {}
    d_user_list["all_user"] = set(user_df["USER_ID_hash"].values)
    d_user_list["detail_user"] = set(train_detail_df["USER_ID_hash"].values)
    d_user_list["view_user"] = set(train_visit_df["USER_ID_hash"].values)
    ...
    #Save to pickle
    with open("../Data/Data_translated/dict_user_list_translated.pickle", "w") as fp:
            pickle.dump(d_user_list, fp)


def create_LabelEncoded_files() :
    ...
    #Save the test data
    test.to_csv("../Data/Data_translated/coupon_list_test_LE_translated.csv", index = False)
    #Free memory
    del test

    #Save the train data
    train.to_csv("../Data/Data_translated/coupon_train_aggregated_LE_translated.csv", index = False)
    #Free memory
    del train
...
def prepare_similarity_data(var_choice):
    """
    ...
    d_user_list = {}
    with open("../Data/Data_translated/dict_user_list_translated.pickle", "r") as fp:
        d_user_list = pickle.load(fp)

    # Load train, test and user list
    test = pd.read_csv("../Data/Data_translated/coupon_list_test_LB_translated.csv")
    train = pd.read_csv("../Data/Data_translated/coupon_train_aggregated_LB_translated.csv")
    ulist = pd.read_csv("../Data/Data_translated/user_list_translated.csv")
    ...
    if var_choice == "1" :
        ...
        for index, trainv in enumerate(pd.read_csv("../Data/Data_translated/coupon_trainv_aggregated_LB_translated.csv", chunksize=100000)) :
            sys.stdout.write("\rProcessing row " + str(index*100000)+" to row "+str((index+1)*100000))
            sys.stdout.flush()
        ...
        # Save data
        uchar.to_csv("../Data/Data_translated/uchar_var_" + str(var_choice) + "_train_translated.csv", index = False)
        test.to_csv("../Data/Data_translated/test_var_" + str(var_choice) + "_translated.csv", index = False)

#########################################
# Below : preprocessing to use LightFM
##########################################

def build_biclass_user_item_mtrx():
    ...
    spi.mmwrite("../Data/Data_translated/biclass_user_item_train_mtrx.mtx", Mui_tr)
...
def build_user_feature_matrix():
    ...
    spi.mmwrite("../Data/Data_translated/user_feat_mtrx.mtx" , ulist)
...
def build_item_feature_matrix():
    ...
    #Save the matrix. They are already in csr format
    spi.mmwrite("../Data/Data_translated/train_item_feat_mtrx.mtx", cpltr)
    spi.mmwrite("../Data/Data_translated/test_item_feat_mtrx.mtx", cplte)
...
if __name__ == "__main__":
    ...
    skim_visit_file()
    create_dict_user_list()
    create_LabelEncoded_files()
    create_LabelBinarized_files()
    var_choice = "1"
    prepare_similarity_data(var_choice)
    #Preprocessing for a hybrid matrix factorisation method
    build_biclass_user_item_mtrx()
